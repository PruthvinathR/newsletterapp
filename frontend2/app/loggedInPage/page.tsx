...
import OutputPage from '../../components/OutputPage'; // Import OutputPage component
...
const newsletters = [
  ...
  { id: 4, name: 'The Skimm', description: 'Daily news summary', categories: ['News', 'Business'], subscribed: false },
  { id: 5, name: 'Hacker Newsletter', description: 'Weekly tech news', categories: ['Tech', 'Development'], subscribed: false },
  { id: 6, name: 'Finimize', description: 'Financial news made simple', categories: ['Finance', 'Investing'], subscribed: false },
  { id: 7, name: 'The Daily Stoic', description: 'Daily philosophy insights', categories: ['Philosophy', 'Self-Improvement'], subscribed: false },
  { id: 8, name: 'The Information', description: 'Tech industry news', categories: ['Tech', 'Business'], subscribed: false },
  { id: 9, name: 'Morning Brew', description: 'Daily business news', categories: ['Business', 'Finance'], subscribed: false },
  { id: 10, name: 'The New York Times', description: 'Daily news and analysis', categories: ['News', 'Culture'], subscribed: false },
...
const logoutAndRedirectToHomepage = (router: ReturnType<typeof useRouter>) => { // Specify the type for router
  console.log("User logged out");
  router.push('/');
};

export default function LoggedInPage() {
const firstName = 'John'; // Default value for firstName
  const lastName = 'Doe'; // Default value for lastName
  const router = useRouter(); // Initialize router
  const [searchTerm, setSearchTerm] = useState('');
  const [activeCategories, setActiveCategories] = useState(['AI']);
  const [activeTab, setActiveTab] = useState('newsletters');
  const [email, setEmail] = useState(`${firstName}.${lastName}@contexti.fyi`); // Use default values for firstName and lastName
  ...
  return (
    <main className="logged-in-page bg-white text-black">
      <header className="flex justify-between items-center p-4 bg-white text-black mx-8">
        <h1 className="text-2xl font-bold">CONTEXTI.FYI</h1>
        <div className="flex space-x-4 flex-grow justify-center"> {/* Adjusted to center Input and Output buttons */}
          <Button variant="outline" onClick={() => setActiveTab('input')}>Input</Button>
          <Button variant="outline" onClick={() => setActiveTab('output')}>Output</Button>
        </div>
        <div className="flex-none"> {/* Account button remains aligned to the right */}
          <Button variant="outline" onClick={() => setIsAccountDialogOpen(true)}>Account</Button>
        </div>
      </header>
      <Dialog open={isAccountDialogOpen} onOpenChange={setIsAccountDialogOpen}>
        <DialogContent className="sm:max-w-[425px] bg-white text-black">
          ...
          <div className="flex flex-col space-y-4">
            ...
            {/* ... existing account management options ... */}
            <Button variant="outline" onClick={() => {
              logoutAndRedirectToHomepage(router); // Pass router to the function
